...
export async function getAllQuotes(
  ...
  const quotes: DeliveryQuote[] = [
    simbaQ.status === 'fulfilled' ? simbaQ.value : { provider: 'simba_courier', key: 'simba_courier', kind: 'courier', title: 'Курьер Simba', description: '', price: 0, daysMin: 0, daysMax: 0, available: false, error: String((simbaQ as PromiseRejectedResult).reason) },
    yandexQ.status === 'fulfilled' ? yandexQ.value : { provider: 'yandex', key: 'yandex_courier', kind: 'courier', title: 'Яндекс Доставка', description: '', price: 0, daysMin: 0, daysMax: 0, available: false, error: String((yandexQ as PromiseRejectedResult).reason) },
    cdekQ.status === 'fulfilled' ? cdekQ.value : { provider: 'cdek', key: 'cdek_pvz', kind: 'courier', title: 'СДЭК', description: '', price: 0, daysMin: 0, daysMax: 0, available: false, error: String((cdekQ as PromiseRejectedResult).reason) },
    ozonQ.status === 'fulfilled' ? ozonQ.value : { provider: 'ozon', key: 'ozon_delivery', kind: 'courier', title: 'Ozon Delivery', description: '', price: 0, daysMin: 0, daysMax: 0, available: false, error: String((ozonQ as PromiseRejectedResult).reason) },
    dostavistaQ.status === 'fulfilled' ? dostavistaQ.value : { provider: 'dostavista', key: 'dostavista_express', kind: 'courier', title: 'Достависта', description: '', price: 0, daysMin: 0, daysMax: 0, available: false, error: String((dostavistaQ as PromiseRejectedResult).reason) },
    postQ.status === 'fulfilled' ? postQ.value : { provider: 'post', key: 'post_parcel', kind: 'courier', title: 'Почта России', description: '', price: 0, daysMin: 0, daysMax: 0, available: false, error: String((postQ as PromiseRejectedResult).reason) },
    getPickupQuote(),
  ]
